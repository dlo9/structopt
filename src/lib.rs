...
pub trait StructOpt {
    ...
    fn from_clap(&clap::ArgMatches, config: Option<&config::Config>) -> Self;

    /// Gets the struct from the command line arguments.  Print the
    /// error message and quit the program in case of failure.
    fn from_args(config: Option<&config::Config>) -> Self
    where
        Self: Sized,
    {
        Self::from_clap(&Self::clap().get_matches(), config)
    }

    /// Gets the struct from any iterator such as a `Vec` of your making.
    /// Print the error message and quit the program in case of failure.
    fn from_iter<I>(iter: I, config: Option<&config::Config>) -> Self
    where
        Self: Sized,
        I: IntoIterator,
        I::Item: Into<OsString> + Clone,
    {
        Self::from_clap(&Self::clap().get_matches_from(iter), config)
    }
    ...
    fn from_iter_safe<I>(iter: I, config: Option<&config::Config>) -> Result<Self, clap::Error>
    where
        Self: Sized,
        I: IntoIterator,
        I::Item: Into<OsString> + Clone,
    {
        Ok(Self::from_clap(&Self::clap().get_matches_from_safe(iter)?, config))
    }
}
